  ...
  return (
    <div className="resultLine">
      <div className="previewContainer">
        <Link to={`/book/${book.key.split('works/')[1]}${book.cover_i*1 > -1 ? `/${book.cover_i}`: '' }${book.cover_edition_key ? `/${book.cover_edition_key}` : ''}`}><img className="previewImage" src={book.cover_i*1 > -1 ? `http://covers.openlibrary.org/b/ID/${book.cover_i}-M.jpg` : '/nia.jpg' } /></Link>
      </div>
      <span className="previewTitle"><Link to={`/book/${book.key.split('works/')[1]}${book.cover_i*1 > -1 ? `/${book.cover_i}`: '' }${book.cover_edition_key ? `/${book.cover_edition_key}` : ''}`}>{book.title}</Link></span>
