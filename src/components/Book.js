...
class Book extends Component {
    ...
    return axios
      ...
      .then( work => {const book = {title: work.title}; book['key'] = work.key ? work.key.split('books/')[1] : ''; book['cover'] = this.props.match.params.cover ? `https://covers.openlibrary.org/b/ID/${this.props.match.params.cover}-L.jpg` : '/nia.jpg'; if(work.identifiers && work.identifiers['amazon.co.uk_asin']){book['amazon'] = work.identifiers['amazon.co.uk_asin'][0]}; if(work.identifiers && work.identifiers['goodreads']){book['goodreads'] = work.identifiers['goodreads'][0]}; if(work.physical_format){book['format'] = work.physical_format}; if(work.publish_date){book['published'] = work.publish_date}; if(work.number_of_pages){book['pages'] = work.number_of_pages}; if(work.subjects){book['subjects'] = work.subjects}; if(work.description){book['description'] = work.description} else {book['description'] = 'No description available.'}; return book})
      .then(book => this.setState({...book}))
      .then(() => {if(this.state.authors.length === 0){return axios.get(`https://openlibrary.org/api/books?bibkeys=${this.state.key ? this.state.key : this.props.match.params.backup ? this.props.match.params.backup : ''}&format=json&jscmd=data`)} else {const empty = {data: {}}; empty.data[this.props.match.params.backup] = {}; return empty}})
      .then(res => (this.state.authors.length === 0 && (this.state.key || this.props.match.params.backup)) ? res.data[this.state.key ? this.state.key : this.props.match.params.backup] : {})
  ...
  componentDidUpdate(prevProps) {
    ...
    if(title !== '') {
      ...
      return (
        ...
        {cover !== '' ? <h2 id="noContent">We're sorry, but there's no data available for this book!<br /><br />It is linked to a different OpenLibrary book, please click on another copy of this title to obtain more info.<br /><br />Sorry for the inconvenience.</h2>:''}
        {cover === '' ? 
          <div id="centerBlock">
            <div class="loading-grid"><div></div><div></div><div></div><div></div><div></div><div></div><div></div><div></div><div></div></div>
          </div>
        : ''}
        </div>
      )
